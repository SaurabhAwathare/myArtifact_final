...
describe("onUserIdentityReset", () => {
  ...
  it("should trigger propagation when identityResetVersion increases", async () => {
    ...
    const change = {
      before: { data: () => beforeData, exists: true } as any,
      after: { data: () => afterData, exists: true } as any
    };

    const wrapped = testEnv.wrap(myFunctions.onUserIdentityReset);

    // Mock Artifacts and Comments query results
    const mockArtifact = { ref: { update: jest.fn() }, data: () => ({ identityVersion: 0 }) };
    const mockComment = { ref: { update: jest.fn() }, data: () => ({ identityVersion: 0 }) };
  ...
  it("should only update documents with OLDER identity version", async () => {
    ...
    const mockRef = { id: "ref1", update: expect.any(Function) };
    const docOld = { ref: mockRef, data: () => ({ identityVersion: 3 }) };
    const docSame = { ref: { id: "ref2", update: expect.any(Function) }, data: () => ({ identityVersion: 5 }) };
    const docNewer = { ref: { id: "ref3", update: expect.any(Function) }, data: () => ({ identityVersion: 6 }) };
    ...
    expect(batchUpdateMock).toHaveBeenCalledWith(mockRef, expect.objectContaining({
